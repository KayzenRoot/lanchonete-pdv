...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (productsCount > 0 && force) {
            ...
            let uncategorizedCategory = await prisma_1.default.category.findFirst({
                where: {
                    OR: [
                        { name: 'Sem categoria' },
                        { name: 'Uncategorized' }
            ...
            if (!uncategorizedCategory) {
                uncategorizedCategory = await prisma_1.default.category.create({
                    data: {
                        name: 'Sem categoria',
        ...
        res.json({
            ...
            details: productsCount > 0 ? (deleteProducts
                ? `Deleted ${productsCount} associated products`
                : `Moved ${productsCount} products to Uncategorized category`) : undefined
        });
    }
    catch (error) {
