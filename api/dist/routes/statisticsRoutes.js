...
router.get('/dashboard', async (req, res) => {
    try {
        console.log(chalk_1.default.blue('📊 Recebendo requisição para dashboard...'));
        const daysParam = req.query.days;
        const days = daysParam ? parseInt(daysParam) : 30;
        console.log(chalk_1.default.blue(`📅 Calculando estatísticas para ${days} dias`));
        // Data atual
        ...
        console.log(chalk_1.default.blue('📊 Buscando vendas do dia...'));
        // Vendas de hoje
        const todaySales = await prisma_1.default.order.aggregate({
            where: {
                createdAt: {
                    gte: today,
                },
        ...
        console.log(chalk_1.default.green(`✅ Vendas hoje: ${todaySales._sum.total || 0}, Quantidade: ${todaySales._count || 0}`));
        // Vendas de ontem
        const yesterdaySales = await prisma_1.default.order.aggregate({
            where: {
                createdAt: {
                    gte: yesterday,
                    lt: today,
        ...
        console.log(chalk_1.default.blue('📊 Buscando vendas da semana...'));
        // Vendas da semana atual
        const weekSales = await prisma_1.default.order.aggregate({
            where: {
                createdAt: {
                    gte: startOfWeek,
                },
        ...
        console.log(chalk_1.default.green(`✅ Vendas da semana: ${weekSales._sum.total || 0}, Quantidade: ${weekSales._count || 0}`));
        // Vendas da semana anterior
        const prevWeekSales = await prisma_1.default.order.aggregate({
            where: {
                createdAt: {
                    gte: startOfPrevWeek,
                    lt: startOfWeek,
        ...
        console.log(chalk_1.default.blue('📊 Buscando vendas do mês...'));
        // Vendas do mês atual
        const monthSales = await prisma_1.default.order.aggregate({
            where: {
                createdAt: {
                    gte: startOfMonth,
                },
        ...
        console.log(chalk_1.default.green(`✅ Vendas do mês: ${monthSales._sum.total || 0}, Quantidade: ${monthSales._count || 0}`));
        // Vendas do mês anterior
        const prevMonthSales = await prisma_1.default.order.aggregate({
            where: {
                createdAt: {
        ...
        console.log(chalk_1.default.blue('📊 Buscando top produtos...'));
        // Itens mais vendidos
        ...
        console.log(chalk_1.default.green(`✅ Encontrados ${topItems.length} produtos mais vendidos`));
        ...
        console.log(chalk_1.default.blue('📊 Buscando pedidos recentes...'));
        // Pedidos recentes
        ...
        console.log(chalk_1.default.green(`✅ Encontrados ${recentOrders.length} pedidos recentes`));
        ...
        console.log(chalk_1.default.blue('📊 Buscando vendas diárias para o gráfico de tendências...'));
        ...
        const dailySalesData = await prisma_1.default.order.groupBy({
            by: ['createdAt'],
            where: {
                createdAt: {
                    gte: startDate,
        ...
        console.log(chalk_1.default.green(`✅ Processados dados de vendas diárias para ${dailySales.length} dias`));
        const dashboardData = {
            sales: {
...
router.get('/reports', async (req, res) => {
    try {
        ...
        const paymentMethods = await prisma_1.default.order.groupBy({
            by: ['paymentMethod'],
            where: {
                createdAt: {
                    gte: startDate,
        ...
        const topProducts = await prisma_1.default.orderItem.groupBy({
            by: ['productId'],
            where: {
                order: {
                    createdAt: {
        ...
        const dailySalesData = await prisma_1.default.$queryRaw `
      SELECT 
...
router.post('/reports', async (req, res) => {
    try {
        ...
        const averageTicket = orderCount > 0 ? totalSales / orderCount : 0;
        // Métodos de pagamento
        const paymentMethodsData = await prisma_1.default.order.groupBy({
            by: ['paymentMethod'],
            where: {
                createdAt: {
                    gte: startDate,
        ...
        // Vendas diárias no período
        const dailySalesRaw = await prisma_1.default.$queryRaw `
      SELECT 
        DATE(createdAt) as date,
        SUM(total) as total
      FROM orders
    ...
    `;
        ...
        try {
            ...
            const previousPeriodData = await prisma_1.default.order.aggregate({
                where: {
                    createdAt: {
...
function getPaymentMethodNameFromKey(key) {
    switch (key) {
        case 'creditCard':
